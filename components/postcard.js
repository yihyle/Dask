...
export default function Postcard({ post }) {
  ...
  return (
    <Link href={`/board/${post.id}`}>
      <div className="w-[1250px] h-[190px] bg-white flex flex-col justify-center rounded-[20px] shadow-lg transition transform duration-300 hover:shadow-xl cursor-pointer gap-4">
        <div className="w-full h-[25px] flex flex-row items-center gap-3">
          ...
          <p className="text-[14px] text-black/50">{post.nickname} - {post.time}</p>
        </div>

        <div className="w-full h-[27px] flex flex-row items-center">
          <p className="text-[22px] text-black font-bold ml-7">{post.title}</p>
        </div>

        <div className="w-full h-[22px] flex flex-row items-center">
          <p className="text-[18px] text-black/70 ml-7">{post.content}</p>
        </div>
        ...
        <div className="w-full h-[15px] flex flex-row items-center">
          <div className="w-auto h-full flex flex-row items-center gap-1 ml-7">
            ...
            <p className="text-[12px] text-black/70">{post.views}</p>
          </div>
          <div className="w-auto h-full flex flex-row items-center gap-1 ml-5">
            ...
            <p className="text-[12px] text-black/70">{post.likes}</p>
          </div>
          <div className="w-auto h-full flex flex-row items-center gap-1 ml-5">
            <Image src="/images/comment.svg" alt="comments" width={17} height={11} className="opacity-70"/>
            <p className="text-[12px] text-black/70">{post.comments}</p>
          </div>
        </div>
      </div>
    </Link>
